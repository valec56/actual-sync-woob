...
# Snapshot all env vars so cron jobs can access them (cron does not inherit the container env).
# export -p produces valid bash with properly quoted values, unlike printenv.
